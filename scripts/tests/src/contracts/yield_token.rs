...
impl YieldTokenInitArgs {
    pub async fn init(self, gov: Arc<TestClient>, addr: Address) -> YieldToken<TestClient> {
        let contract = YieldToken::new(addr, gov.clone());

        contract
            .init(self.name, self.symbol, self.minter, self.initial_supply)
            .await
            .unwrap();
