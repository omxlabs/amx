...
use crate::{
    constants::{
        ATOM_DECIMALS, BNB_DECIMALS, BTC_DECIMALS, ETH_DECIMALS, OSMO_DECIMALS, USDC_DECIMALS,
        USDT_DECIMALS,
    },
    contracts::{
...
impl VaultContracts {
    pub async fn set_token_config(
        ...
        send(self.vault.set_token_config(
            token,
            token_decimals,
            token_weight,
...
pub struct VaultContracts {
    pub vault: Vault<LiveClient>,
    pub funding_rate_manager: FundingRateManager<LiveClient>,
    pub swap_manager: SwapManager<LiveClient>,
    pub fee_manager: FeeManager<LiveClient>,
    pub positions_manager: PositionsManager<LiveClient>,
    pub positions_manager_utils: PositionsManagerUtils<LiveClient>,
    pub positions_decrease_manager: PositionsDecreaseManager<LiveClient>,
    pub positions_increase_manager: PositionsIncreaseManager<LiveClient>,
    pub positions_liquidation_manager: PositionsLiquidationManager<LiveClient>,
    pub vault_utils: VaultUtils<LiveClient>,
}

impl VaultContractsInitArgs {
    /// Initialize all vault contracts
    pub async fn init(self, ctx: &DeployContext, contracts: &ContractAddresses) -> VaultContracts {
        println!("initializing vault contracts");
        ...
        VaultContracts {
            ...
            .init(&ctx, contracts.vault.vault)
            .await,
            ...
            .init(&ctx, contracts.vault.vault_utils)
            .await,
            ...
            .init(&ctx, contracts.vault.fee_manager)
            .await,
            ...
            .init(&ctx, contracts.vault.funding_rate_manager)
            .await,
            ...
            .init(&ctx, contracts.vault.positions_decrease_manager)
            .await,
            ...
            .init(&ctx, contracts.vault.positions_increase_manager)
            .await,
            ...
            .init(&ctx, contracts.vault.positions_manager)
            .await,
            ...
            .init(&ctx, contracts.vault.positions_manager_utils)
            .await,
            ...
            .init(&ctx, contracts.vault.positions_liquidation_manager)
            .await,
            ...
            .init(&ctx, contracts.vault.swap_manager)
            .await,
        }
    }
}
